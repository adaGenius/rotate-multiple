...
- (void)doStableConstraint {
    ...
    [self.durationLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.centerY.equalTo(self.bottomBarView);
        make.width.mas_equalTo(self.durationLabel.bounds.size.width);
        make.height.mas_equalTo(self.durationLabel.bounds.size.height);
    }];
...
- (void)clickExitFullScreenButton {
    //    [self transformWithOrientation:UIDeviceOrientationPortrait];
    if ([self.delegate respondsToSelector:@selector(rotateToProtrait:)]) {
        [self.delegate rotateToProtrait:0];
    }
...
- (void)clickEnterFullScreenButton {
    ...
    if (UIDeviceOrientationLandscapeRight == [[UIDevice currentDevice]orientation]) {
        if ([self.delegate respondsToSelector:@selector(rotateToProtrait:)]) {
            [self.delegate rotateToProtrait:1];
        }
    } else {
        if ([self.delegate respondsToSelector:@selector(rotateToProtrait:)]) {
            [self.delegate rotateToProtrait:2];
...
/// 设备方向改变的处理 只是演示demo的一种方法，实际情况根据自己需要更改
- (void)applicationWillChangeStatusBarOrientationNotification:(NSNotification *)notification {
    ...
    [self transformWithOrientation:orientation];
}
